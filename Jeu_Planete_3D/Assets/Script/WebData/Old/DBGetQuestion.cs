...
public class DBGetQuestion : MonoBehaviour
{
    ...
    [FormerlySerializedAs("ShowQcm")] public ShowQCU<> showQcu;
    [FormerlySerializedAs("ShowInterac")] public ShowManipulation<> showManipulation;
    [FormerlySerializedAs("ShowVraiFaux")] public ShowTrueFalse<> showTrueFalse;
    ...
    public IEnumerator GetQuestion(int qid)
    {
        ...
        if (wwwInteract.error != null)
        {
            ...
            /* In case of emergency if its impossible to connect to the host since the start,
             read the expected html page content for a known question and store the value for later used#1#
        ...
        if (type.Equals("QCM"))
        {
            ...
            showQcu.showQuestion(htmlParser.GetHTML());
        }
        else if (type.Equals("QUESINTERAC"))
        {
            panelInterac.SetActive(true);
            showManipulation.showQuestion(htmlParser.GetHTML());
        }
        else if (type.Equals("VRAIFAUX"))
        {
            panelVraiFaux.SetActive(true);
            showTrueFalse.showQuestion(htmlParser.GetHTML());
        }
